...
import { useStore } from "vuex";
  ...
  onMounted(() => {
    setCurrentPageBreadcrumbs("Materi File", ['LMS', 'Materi']);
    // getData()
  })
  ...
  function getMateriData (payload) {
    request.post('materi/file', QueryString.stringify({
      ...
      materiData.rows = res.data.data.materi
      materiData.totalRows = res.data.total
    }).catch(err => {
      materiData.rows = []
    })
  }
      ...
      <div class="page-content">
        <div class="d-flex flex-wrap justify-content-between align-items-center">
          <div class="d-flex gap-4">
            <h2 class="fs-1 fw-bold py-6">Data Materi File</h2>
          </div>
        </div>
          ...
          >
            <template #table-row="{column, row}">
              <div v-if="column.field == 'materi_file'">
                <i :class="`text-primary bi bi-file-earmark-font-fill`"></i>
                {{row.materi_file}}
              </div>
              <div v-if="column.field == 'materi_filesize'">
                {{formatBytes(row.materi_filesize)}}
              </div>
              <div v-if="column.field == 'materi_create_date'">
                {{dateFormating(row.materi_create_date)}}
              </div>
              <div v-if="column.field == 'action'">
                <a :href="`${storageUrl}/${currentUser.sekolah_kode}/apischool/public` + '/files/' + row.materi_file" target="_blank" class="btn btn-bg-light btn-active-color-primary btn-sm me-2">
                  <span class="svg-icon svg-icon-3">
                    <i :class="`bi bi-cloud-arrow-down fs-2`"></i>{{'Download'}} 
                  </span>
                </a>
              </div>
            </template>
