          ...
          <ServerSideTable 
            ...
            <template #table-row="{column, row}">
              ...
              <div v-if="column.field == 'action'">
                <template v-if="row.notification_type == 'Academic' || row.notification_type == 'Event'">
                  <router-link :to="`/sekolah/informasi/event/detail/${row.notification_id}`" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
                    <span class="svg-icon svg-icon-3">
                      <i class="bi bi-eye-fill fs-3"></i>
                    </span>
                  </router-link>
                </template>
                <template v-if="row.notification_type == 'Announcement'">
                  <router-link :to="`/sekolah/informasi/pengumuman/detail/${row.notification_id}`" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
                    <span class="svg-icon svg-icon-3">
                      <i class="bi bi-eye-fill fs-3"></i>
                    </span>
                  </router-link>
